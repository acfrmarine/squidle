...
class PointAnnotationSetResource(ModelResource):
    ...
    def prepend_urls(self):
        return [
                url(r"^(?P<resource_name>%s)/(?P<set_id>\w[\w/-]*)/(?P<image_id>\w[\w/-]*)%s$" % (self._meta.resource_name, trailing_slash()), self.wrap_view('get_image_annotations'), name="api_get_image_annotations"),
            ]

    def get_image_annotations(self, request, **kwargs):
        set_id = kwargs['set_id']
        image_id = kwargs['image_id']

        # this is the number you are after - there doesn't appear to be a great way to get it
        # from the api, although querying point_annotations with:
        # /api/dev/point_annotations/?annotation_set=id&image=id&label__ne=1
        # may work... (with slight tweaking of field names etc.) just take the total out of the
        # list it provides
        count = models.PointAnnotation.objects.filter(annotation_set=set_id, image=image_id).exclude(label=1).count()

        # this gets the annotation resource we want to use to serialize etc.
        point_annotations = PointAnnotationResource()

        # unfortunately calling get_list does a lot of stuff and gives us the output
        # request - so no modification of the data at that point and we want to insert in
        # the top level anyway above the list in the header section that usually comes with it

        import types
        def alter_list(self, request, data):
            # you should be able to insert fields into it at this point at the top level - if you
            # see the json output its at the same level as the count, next, prev fields. If you set
            # value there, and don't want the annotations themselves you can set the limit to 0 or
            # another low number and get what you are after
            data['labelled'] = count
            return data

        # this magic changes the alter list function on the resource to the one that was just
        # created. I haven't tested it, but it should work as intended...
        # it could be overriden globally, but doesn't make sense, and is tricky to get the count
        # into.
        point_annotations.alter_list_data_to_serialize = types.MethodType(alter_list, point_annotations)

        return point_annotations.get_list(request, image=image_id, annotation_set=set_id)
