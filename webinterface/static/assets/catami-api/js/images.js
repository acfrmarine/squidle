function ImagesAPI(usrsettings) {
    var settings = {
        api_baseurl: '/api/dev/image/',
        linkurl: "/imageannotate/",
        api_infourl: '/api/dev/pose/'
    }
     ...
     * @return {String}
     */
    this.getThumnailList = function (apistring, addurl) {
        if (addurl == true) apistring = settings.api_baseurl + "?" + apistring;
        var parent = this;
        var obj = {};
        var thlist = '';
        ajaxobj = $.ajax({
            ...
            success: function (img) {
                if (img.objects.length > 0) {
                    for (var i = 0; i < img.objects.length; i++) {
                        ...
                        thlist+= formatThObj(config.format, obj);

                    }
                }
                else {
                    thlist += '<p class="ajax-empty">There are no images to show. Select or create a dataset by <br>clicking the dropdown menu on the top navigation bar.</p>';
                }
    ...
    this.appendThumnails = function (outputelement, apistring) {
        $(outputelement).append(this.getThumnailList(apistring));
    }
    ...
    this.getImageInfo = function (imgid) {
        var apistring = settings.api_infourl+'?images='+imgid,
            imginfo = {};
        ajaxobj = $.ajax({
            dataType: "json",
            async: false,  // prevent asyncronous mode to allow setting of variables within function
    ...
    function getThObj(obj) {
        return objout = {
            ...
            annotation_count: obj.hasOwnProperty('annotation_count') ? obj.annotation_count : 0,
            annotation_labelled_count: obj.hasOwnProperty('annotation_labelled_count') ? obj.annotation_labelled_count : 0
        };
    }
