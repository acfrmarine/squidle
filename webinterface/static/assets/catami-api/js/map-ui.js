...
//        ));
		...
		this.mapInstance.events.on({
			"zoomend": function(e) {
				...
				if( baseMap.mapInstance.getLayersByName(baseMap.depImageLayerName)[0].inRange && !baseMap.mapInstance.getControlsBy('id','showFeatureInfoCtrl')[0].active ) {
					console.log('activating');
					baseMap.mapInstance.getControlsBy('id','showFeatureInfoCtrl')[0].activate();
				} else if( !baseMap.mapInstance.getLayersByName(baseMap.depImageLayerName)[0].inRange && baseMap.mapInstance.getControlsBy('id','showFeatureInfoCtrl')[0].active ) {
        ...
        deploymentlayer.events.on({
			"featureadded" : function(evt) {
				...
				if( evt.feature.cluster.length === 1 ) {
					campaigns = false;
					namesU = baseMap.getDeploymentsFromClusterFeature(evt.feature);
				}
				// Get the name of the campaigns within the deployment cluster
				else {
					campaigns = true;
					names = baseMap.getCampaignsFromClusterFeature(evt.feature);
					...
					for(i = 0; i < namesU.length; i++) {
						depName = namesU[i];
						sum = 0;
						inThis = evt.feature.cluster.map( function(e) { return (e.data.campaign_name === depName) ? 1 : 0; } );
						...
						$($c[i]).tooltip({
							...
							'title': function() {
								...
								if( selected ) { //&& $currTitle.find('.tooltip-buttons').length === 0) {
									$currTitle.find('.tooltip-buttons').css('display','block');
								}
								else {
									$currTitle.find('.tooltip-buttons').css('display','none');
								}
