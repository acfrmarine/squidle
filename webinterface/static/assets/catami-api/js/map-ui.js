	...
	this.addDeployments = function(layername) {
        ...
        deploymentlayer.events.on({
			"featureadded" : function(evt) {
				...
				campsU = camps.filter( function(itm,i,a) {
					return i==a.indexOf(itm);
				}).sort().join('<br>');

				// Create the content of the tooltip when the circle is generated by the map
				var $btnZoom = $('<button type="button" id="btnZoom" class="btn btn-xs btn-group btn-group-xs" title="Zoom into deployments."><i class="icon-zoom-in"></i>&nbsp;Zoom</button>');
				var $btnList = $('<button type="button" id="btnList" class="btn btn-xs btn-group btn-group-xs" title="List deployments."><i class="icon-list"></i>&nbsp;List</button>');
				...
				var $divBtns = $('<div id="tooltip_buttons" class="tooltip-buttons"></span>');
				$divBtns.append($btnZoom, $btnList);
				
				var $divNames = $('<div id="tooltip_campaign_names"></div>');
				$divNames.append( campsU );
				
				$toolHTML = $('<div id="tooltip_'+circId.split('_')[1]+'"></div>');
				$toolHTML.append($divNames);
				
				// Find the circle that has this ID and attach the tooltip to it
				$c = $('circle');
					...
					if( circId === $c[i].id ) {
						...
						$($c[i]).tooltip({
							...
							'title': function() {
								...
								selected = $($c[i]).data('selected');
								$currTitle = $($($c[i]).data('title'));
								console.log($currTitle);
								// If selected and missing buttons
								if( selected && $currTitle.find('.tooltip-buttons').length === 0) {
									console.log('adding buttons');
									$currTitle.append($divBtns);
								}
								// If hover but having buttons
								else if( !selected && $currTitle.find('.tooltip-buttons').length === 1 ) {
									console.log('removing buttons');
									// $currTitle.remove('#tooltip-buttons');
								}
